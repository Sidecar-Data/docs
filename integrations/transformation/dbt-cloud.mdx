...
## Enable MCP for agents

To give Agent Console agents live dbt Cloud investigation and retry tools,
follow [dbt Cloud MCP](/integrations/mcp-servers/dbt-cloud).

MCP setup requires an **Access URL** saved on the **Connection** tab, even for
accounts that use standard URLs. Connecting the integration alone does not
enable MCP or grant it to an agent.
